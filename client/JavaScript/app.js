let app = new Vue({
  ...
  methods: {
    ...
    fetch("http://localhost:3000/order12",
    {
        method: "POST",
        body: data
    })
    ...
    .then(function(data){ alert( JSON.stringify( data ) ) });
          ...
          fetch("http://localhost:3000/updateinformation",
          {
              method: "PUT",
              body: data1
          })
...
alert("fill in form");
  ...
  lessonleft=  app.subjects[app.cart.id].spaces - 1;
  fetch("http://localhost:3000/order?id="+app.cart.id+"&name="+app.Nameofu+"&phonenumber="+app.phoneofu+"&spaces="+lessonleft).then(function (response) {
  ...
  fetch("http://localhost:3000/update?subject="+app.subjects[app.cart.id].subject+"&spaceleft="+lessonleft).then(function (response) {
    response.json().then(function (json) {
     console.log("sent order");
